...
class InsentifImport extends Component
{
    use WithFileUploads;
    ...
    {
        return view('livewire.dashboard.insentif.insentif-import');
    }
    ...
    public function store()
    {
        ...
        foreach ($this->employees as $employeeData) {
            ...
            $existingAbsence = Insentif::where([
            ...
            if ($data) {
                $this->dispatch('insentif-imported');
                $this->dispatch('close-modal', name: 'import-insentif-modal');
                $this->reset();
                $this->resetValidation();
                $this->dispatch('alert-success', title: 'Total Amount Insentif Successfully Imported!');
            } else {
                $this->dispatch('alert-failure', title: 'Failed to Import Insentif');
            }
