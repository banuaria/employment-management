...
class AbsentIndex extends Component
{
    ...
    {
        $absentMonthly = AbsentMonthlies::with(['employeeMaster','vendors'])
        ...
        ->paginate(10);
        $data = [
            'absentMonthly' => $absentMonthly,
            'vendors' => Vendor::all(),
        ];

        return view('livewire.dashboard.absent.absent-index', $data)->layout('layouts.dashboard', [
            'header' => 'Absent'
        ]);
    }
    ...
    public function export()
    {
        ...
        $fileName = 'absent-employee-' . $date . '.xlsx'; // Append the date to the file name
    ...
    {
        $emp = AbsentMonthlies::destroy($id);
        if ($emp) {
