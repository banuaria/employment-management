...
class LainyaImport extends Component
{
    use WithFileUploads;
    ...
    {
        return view('livewire.dashboard.lainya.lainya-import');
    }
    ...
    public function store()
    {
        ...
        foreach ($this->employees as $employeeData) {
            ...
            if ($data) {
                $this->dispatch('lainya-imported');
                $this->dispatch('close-modal', name: 'import-lainya-modal');
                $this->reset();
                $this->resetValidation();
                $this->dispatch('alert-success', title: 'Total Amount lainya Successfully Imported!');
            } else {
                $this->dispatch('alert-failure', title: 'Failed to Import lainya');
            }
