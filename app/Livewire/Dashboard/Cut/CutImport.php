...
class CutImport extends Component
{
    use WithFileUploads;
    ...
    {
        return view('livewire.dashboard.cut.cut-import');
    }
    ...
    public function store()
    {
        ...
        foreach ($this->employees as $employeeData) {
            ...
            if ($data) {
                $this->dispatch('cut-imported');
                $this->dispatch('close-modal', name: 'import-cut-modal');
                $this->reset();
                $this->resetValidation();
                $this->dispatch('alert-success', title: 'Total Amount cutsalary Successfully Imported!');
            } else {
                $this->dispatch('alert-failure', title: 'Failed to Import cutsalary');
            }
