...
class CleaningImport extends Component
{
    use WithFileUploads;
    ...
    {
        return view('livewire.dashboard.cleaning.cleaning-import');
    }
    ...
    public function store()
    {
        ...
        foreach ($this->employees as $employeeData) {
            ...
            if ($data) {
                $this->dispatch('cleaning-imported');
                $this->dispatch('close-modal', name: 'import-cleaning-modal');
                $this->reset();
                $this->resetValidation();
                $this->dispatch('alert-success', title: 'Total Amount Cleaning Successfully Imported!');
            } else {
                $this->dispatch('alert-failure', title: 'Failed to Import Cleaning');
            }
