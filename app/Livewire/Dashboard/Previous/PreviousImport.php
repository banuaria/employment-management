...
class PreviousImport extends Component
{
    use WithFileUploads;
    ...
    {
        return view('livewire.dashboard.previous.previous-import');
    }
    ...
    public function store()
    {
        ...
        foreach ($this->employees as $employeeData) {
            ...
            if ($data) {
                $this->dispatch('previous-imported');
                $this->dispatch('close-modal', name: 'import-previous-modal');
                $this->reset();
                $this->resetValidation();
                $this->dispatch('alert-success', title: 'previous Successfully Imported!');
            } else {
                $this->dispatch('alert-failure', title: 'Failed to Import previous');
            }
