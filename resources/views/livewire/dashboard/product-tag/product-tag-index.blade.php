<div class="py-12">
        ...
        <div class="p-4 sm:p-8 bg-white shadow rounded-lg">
            <div class="w-full">
                <div class="flex justify-between items-center space-x-4 mb-4">
                    <div>
                        <button wire:click="$dispatchTo('dashboard.product-tag.product-tag-create', 'created')" type="button" class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest transition ease-in-out duration-1500">Create Tag</button>
                    </div>
                            ...
                            <tbody>
                                @if (count($product_tags) > 0)
                                    ...
                                    <tr class="bg-white border-b">
                                        <td class="px-4 py-3 border text-center w-0"><div class="py-1.5">{{ $product_tags->firstItem() + $key }}</div></td>
                                        <td class="px-4 py-3 border-r">{{ $product_tag->title }}</td>
                                        <td class="px-4 py-3 border-r">
                                            <div class="whitespace-nowrap">{{ $product_tag->productCategory->title ?? '' }}</div>
                                        </td>
                                        <td class="px-4 py-3 border-r w-0 whitespace-nowrap">
                                            <div class="flex flex-col justify-center items-center space-y-2">
                                                <div>{{ $product_tag->createdBy->name }}</div>
                                                <div>{{ $product_tag->created_at->format('d-m-Y H:i') }}</div>
                                            </div>
                                        </td>
                                        <td class="px-4 py-3 border-r w-0 whitespace-nowrap">
                                            <div class="flex flex-col justify-center items-center space-y-2">
                                                <div>{{ $product_tag->updatedBy->name }}</div>
                                                <div>{{ $product_tag->updated_at->format('d-m-Y H:i') }}</div>
                                            </div>
                                        </td>
                                        <td class="px-4 py-3 border-r w-0">
                                            <div class="flex justify-center items-center">
                                                <label class="relative cursor-pointer">
                                                    <input type="checkbox" value="" class="sr-only peer" wire:change="updateStatus({{ $product_tag->id }}, $event.target.checked)" {{ $product_tag->status ? 'checked' : ''}}>
                                        ...
                                        <td class="px-4 py-2 border-r w-0">
                                            <div class="flex justify-center items-center space-x-2">
                                                <button wire:click="$dispatchTo('dashboard.product-tag.product-tag-edit', 'edited', { id: {{ $product_tag->id }} })" type="button" class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest transition ease-in-out duration-1500"><x-icon-edit class="w-3.5 h-3.5"></x-icon-edit></button>
                                                <button wire:click="$dispatch('alert-confirmation', { to: 'dashboard.product-tag.product-tag-index', data: { do: 'delete', 'id': {{ $product_tag->id }} }, title: `Are you sure to delete tag {{ $product_tag->title }}?` })" type="button" class="inline-flex items-center px-4 py-2 bg-red-600 hover:bg-red-500 active:bg-red-700 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest transition ease-in-out duration-1500"><x-icon-trash class="w-3.5 h-3.5"></x-icon-trash></button>
                                            </div>
                                        </td>
                                    </tr>
                                    @endforeach
                ...
                <div class="{{ $product_tags->hasPages() ? 'mt-4' : '' }}">
                    {{ $product_tags->links() }}
                </div>
            </div>
        </div>
    ...
    <x-modal name="create-product-tag-modal" closable="false" maxWidth="lg">
        <div class="flex justify-between items-start mb-4 space-x-4">
            <h4 class="font-semibold text-gray-600 pt-1 uppercase">
                Create Tag
            </h4>
            <button
                wire:click="$dispatch('close-modal', { name: 'create-product-tag-modal' })"
                type="button"
        ...
        <div class="flex-1 overflow-auto x-modal-content">
            <livewire:dashboard.product-tag.product-tag-create :product_categories="$product_categories" @productTagCreated="$refresh" />
        </div>
    </x-modal>

    <x-modal name="edit-product-tag-modal" closable="false" maxWidth="lg">
        <div class="flex justify-between items-start mb-4 space-x-4">
            <h4 class="font-semibold text-gray-600 pt-1 uppercase">
                Edit Tag
            </h4>
            <button
                wire:click="$dispatch('close-modal', { name: 'edit-product-tag-modal' })"
                type="button"
                class="text-gray-600 hover:text-gray-900 bg-gray-50 hover:bg-gray-100 p-2 rounded-full inline-flex items-center"
            >
                <x-icon-close class="w-4 h-4"></x-icon-close>
        ...
        <div class="flex-1 overflow-auto x-modal-content">
            <livewire:dashboard.product-tag.product-tag-edit :product_categories="$product_categories" @productTagEdited="$refresh" />
        </div>
    </x-modal>
</div>
