        ...
        <div class="p-4 sm:p-8 bg-white shadow rounded-lg">
            <div class="w-full">
                <div class="flex justify-between items-center space-x-4 mb-4">
                    <div>
                        <button wire:click="$dispatchTo('dashboard.post-subcategory.post-subcategory-create', 'created')" type="button" class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest transition ease-in-out duration-1500">Create Subcategory</button>
                    </div>
                    <div class="flex-1 flex flex-col sm:flex-row justify-end items-end space-x-0 sm:space-x-2 space-y-2 sm:space-y-0">
                        ...
                        <div class="relative w-full max-w-48">
                            <select wire:model.live.debounce.250ms="post_category_id" class="block w-full p-2 text-sm text-gray-900 border border-gray-300 rounded-lg bg-white focus:ring-indigo-500 focus:border-indigo-500">
                            ...
                            <tbody>
                                @if (count($post_subcategories) > 0)
                                    ...
                                    <tr class="bg-white border-b">
                                        <td class="px-4 py-3 border text-center w-0"><div class="py-1.5">{{ $post_subcategories->firstItem() + $key }}</div></td>
                                        <td class="px-4 py-3 border-r">{{ $post_subcategory->title }}</td>
                                        <td class="px-4 py-3 border-r">
                                            <div class="whitespace-nowrap">{{ $post_subcategory->postCategory->title ?? '' }}</div>
                                        </td>
                                        <td class="px-4 py-3 border-r w-0 whitespace-nowrap">
                                            <div class="flex flex-col justify-center items-center space-y-2">
                                                <div>{{ $post_subcategory->createdBy->name }}</div>
                                                <div>{{ $post_subcategory->created_at->format('d-m-Y H:i') }}</div>
                                            </div>
                                        </td>
                                        <td class="px-4 py-3 border-r w-0 whitespace-nowrap">
                                            <div class="flex flex-col justify-center items-center space-y-2">
                                                <div>{{ $post_subcategory->updatedBy->name }}</div>
                                                <div>{{ $post_subcategory->updated_at->format('d-m-Y H:i') }}</div>
                                            </div>
                                        </td>
                                        <td class="px-4 py-3 border-r w-0">
                                            <div class="flex justify-center items-center">
                                                <label class="relative cursor-pointer">
                                                    <input type="checkbox" value="" class="sr-only peer" wire:change="updateStatus({{ $post_subcategory->id }}, $event.target.checked)" {{ $post_subcategory->status ? 'checked' : ''}}>
                                        ...
                                        <td class="px-4 py-2 border-r w-0">
                                            <div class="flex justify-center items-center space-x-2">
                                                <button wire:click="$dispatchTo('dashboard.post-subcategory.post-subcategory-edit', 'edited', { id: {{ $post_subcategory->id }} })" type="button" class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest transition ease-in-out duration-1500"><x-icon-edit class="w-3.5 h-3.5"></x-icon-edit></button>
                                                <button wire:click="$dispatch('alert-confirmation', { to: 'dashboard.post-subcategory.post-subcategory-index', data: { do: 'delete', 'id': {{ $post_subcategory->id }} }, title: `Are you sure to delete subcategory {{ $post_subcategory->title }}?` })" type="button" class="inline-flex items-center px-4 py-2 bg-red-600 hover:bg-red-500 active:bg-red-700 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest transition ease-in-out duration-1500"><x-icon-trash class="w-3.5 h-3.5"></x-icon-trash></button>
                                            </div>
                                        </td>
                                    </tr>
                                    @endforeach
                ...
                <div class="{{ $post_subcategories->hasPages() ? 'mt-4' : '' }}">
                    {{ $post_subcategories->links() }}
                </div>
            </div>
        </div>
    </div>

    <x-modal name="create-post-subcategory-modal" closable="false" maxWidth="lg">
        <div class="flex justify-between items-start mb-4 space-x-4">
            <h4 class="font-semibold text-gray-600 pt-1 uppercase">
                Create Subcategory
            </h4>
            <button
                wire:click="$dispatch('close-modal', { name: 'create-post-subcategory-modal' })"
                type="button"
        ...
        <div class="flex-1 overflow-auto x-modal-content">
            <livewire:dashboard.post-subcategory.post-subcategory-create :post_categories="$post_categories" @postSubcategoryCreated="$refresh" />
        </div>
    </x-modal>

    <x-modal name="edit-post-subcategory-modal" closable="false" maxWidth="lg">
        <div class="flex justify-between items-start mb-4 space-x-4">
            <h4 class="font-semibold text-gray-600 pt-1 uppercase">
                Edit Subcategory
            </h4>
            <button
                wire:click="$dispatch('close-modal', { name: 'edit-post-subcategory-modal' })"
                type="button"
                class="text-gray-600 hover:text-gray-900 bg-gray-50 hover:bg-gray-100 p-2 rounded-full inline-flex items-center"
            >
                <x-icon-close class="w-4 h-4"></x-icon-close>
        ...
        <div class="flex-1 overflow-auto x-modal-content">
            <livewire:dashboard.post-subcategory.post-subcategory-edit :post_categories="$post_categories" @postSubcategoryEdited="$refresh" />
        </div>
    </x-modal>
</div>
