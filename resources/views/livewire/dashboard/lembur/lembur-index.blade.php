<div class="py-12">
            ...
            <div class="w-full">
                <div class="flex justify-between items-center space-x-4 mb-4">
                    <div>
                        ...
                        <button wire:click="$dispatch('open-modal', { name: 'import-lembur-modal' })" type="button" class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest transition ease-in-out duration-1500">Import</button>
                            ...
                            <tbody>
                                @if (count($lembur) > 0)
                                    ...
                                    <tr class="bg-white border-b">
                                        <td class="px-4 py-3 border text-center w-0"><div class="py-1.5">{{ $lembur->firstItem() + $key }}</div></td>
                ...
                <div class="{{ $lembur->hasPages() ? 'mt-4' : '' }}">
                    {{ $lembur->links() }}
                </div>
            </div>
        </div>
    ...
    <x-modal name="import-lembur-modal" closable="false" maxWidth="lg">
        <div class="flex justify-between items-start mb-4 space-x-4">
            <h4 class="font-semibold text-gray-600 pt-1 uppercase">
                Import Lembur
            </h4>
            <button
                wire:click="$dispatch('close-modal', { name: 'import-lembur-modal' })"
                type="button"
        ...
        <div class="flex-1 overflow-auto x-modal-content">
            <livewire:dashboard.lembur.lembur-import @lemburImported="$refresh" />
        </div>
    </x-modal>

    <x-modal name="edit-lembur-modal" closable="false" maxWidth="lg">
        <div class="flex justify-between items-start mb-4 space-x-4">
            <h4 class="font-semibold text-gray-600 pt-1 uppercase">
                Edit Lembur 
            </h4>
            <button
                wire:click="$dispatch('close-modal', { name: 'edit-lembur-modal' })"
                type="button"
                class="text-gray-600 hover:text-gray-900 bg-gray-50 hover:bg-gray-100 p-2 rounded-full inline-flex items-center"
            >
                <x-icon-close class="w-4 h-4"></x-icon-close>
        ...
        <div class="flex-1 overflow-auto x-modal-content">
            <livewire:dashboard.lembur.lembur-edit @lemburEdited="$refresh" />
        </div>
    </x-modal>
</div>
