<div>
    <div class="sm:px-6 lg:px-8">
        <div class="p-4 sm:p-8 bg-white shadow rounded-lg">
            <div class="text-center font-bold text-xl">Covers</div>
            <div class="w-full">
                <div class="flex justify-between items-center space-x-4 mb-4">
                    @if (count($covers) < 10)
                    <div>
                        <button wire:click="$dispatchTo('dashboard.cover.cover-create', 'created')" type="button" class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest transition ease-in-out duration-1500">Add Cover</button>
                    </div>
                    @endif
                        ...
                        <table class="w-full text-sm text-left rtl:text-right text-gray-500">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                                <tr>
                                    ...
                                    <th rowspan="1" class="px-6 py-3 border text-center whitespace-nowrap">Url</th>
                            ...
                            <tbody>
                                @if (count($covers) > 0)
                                    ...
                                    <tr class="bg-white border-b">
                                        <td class="px-4 py-3 border text-center w-0"><div class="py-1.5">{{ $covers->firstItem() + $key }}</div></td>
                                        <td class="px-4 py-3 border-r w-0">
                                            <div class="w-48 h-auto">
                                                <img src="{{ asset($cover->path) }}" class="w-full h-full object-cover" />
                                            </div>
                                        </td>
                                        <td class="px-4 py-3 border-r">
                                            <a href={{ $cover->url }} target="_blank" class="mt-1 text-sm text-blue-500">
                                                {{ $cover->url }}
                                            </a>
                                        </td>
                                        <td class="px-4 py-3 border-r w-0 whitespace-nowrap">
                                            <div class="flex flex-col justify-center items-center space-y-2">
                                                <div>{{ $cover->createdBy->name }}</div>
                                                <div>{{ $cover->created_at->format('d-m-Y H:i') }}</div>
                                            </div>
                                        </td>
                                        <td class="px-4 py-3 border-r w-0 whitespace-nowrap">
                                            <div class="flex flex-col justify-center items-center space-y-2">
                                                <div>{{ $cover->updatedBy->name }}</div>
                                                <div>{{ $cover->updated_at->format('d-m-Y H:i') }}</div>
                                            </div>
                                        </td>
                                        <td class="px-4 py-3 border-r w-0">
                                            <div class="flex justify-center items-center">
                                                <label class="relative cursor-pointer">
                                                    <input type="checkbox" value="" class="sr-only peer" wire:change="updateStatus({{ $cover->id }}, $event.target.checked)" {{ $cover->status ? 'checked' : ''}}>
                                        ...
                                        <td class="px-4 py-2 border-r w-0">
                                            <div class="flex justify-center items-center space-x-2">
                                                <button wire:click="$dispatchTo('dashboard.cover.cover-edit', 'edited', { id: {{ $cover->id }} })" type="button" class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest transition ease-in-out duration-1500"><x-icon-edit class="w-3.5 h-3.5"></x-icon-edit></button>
                                                <button wire:click="$dispatch('alert-confirmation', { to: 'dashboard.cover.cover-index', data: { do: 'delete', 'id': {{ $cover->id }} }, title: `Are you sure to delete cover?` })" type="button" class="inline-flex items-center px-4 py-2 bg-red-600 hover:bg-red-500 active:bg-red-700 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest transition ease-in-out duration-1500"><x-icon-trash class="w-3.5 h-3.5"></x-icon-trash></button>
                                            </div>
                                        </td>
                                    </tr>
                                    @endforeach
                ...
                <div class="{{ $covers->hasPages() ? 'mt-4' : '' }}">
                    {{ $covers->links() }}
                </div>
            </div>
        </div>
    ...
    <x-modal name="create-cover-modal" closable="false" maxWidth="lg">
        <div class="flex justify-between items-start mb-4 space-x-4">
            <h4 class="font-semibold text-gray-600 pt-1 uppercase">
                Add Cover
            </h4>
            <button
                wire:click="$dispatch('close-modal', { name: 'create-cover-modal' })"
                type="button"
        ...
        <div class="flex-1 overflow-auto x-modal-content">
            <livewire:dashboard.cover.cover-create @coverCreated="$refresh" />
        </div>
    </x-modal>

    <x-modal name="edit-cover-modal" closable="false" maxWidth="lg">
        <div class="flex justify-between items-start mb-4 space-x-4">
            <h4 class="font-semibold text-gray-600 pt-1 uppercase">
                Edit Cover
            </h4>
            <button
                wire:click="$dispatch('close-modal', { name: 'edit-cover-modal' })"
                type="button"
                class="text-gray-600 hover:text-gray-900 bg-gray-50 hover:bg-gray-100 p-2 rounded-full inline-flex items-center"
            >
                <x-icon-close class="w-4 h-4"></x-icon-close>
        ...
        <div class="flex-1 overflow-auto x-modal-content">
            <livewire:dashboard.cover.cover-edit @coverEdited="$refresh" />
        </div>
    </x-modal>
</div>
