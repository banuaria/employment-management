<div class="py-12">
    <div class="sm:px-6 lg:px-8">
        <div class="p-4 sm:p-8 bg-white shadow rounded-lg">
            <div class="w-full">
                ...
                <div class="relative">
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left rtl:text-right text-gray-500">
                            ...
                            <tbody>
                                @if (count($absentMonthly) > 0)
                                    @foreach ($absentMonthly as $key => $value)
                                    <tr class="bg-white border-b">
                                        <td class="px-4 py-3 border text-center w-0"><div class="py-1.5">{{ $absentMonthly->firstItem() + $key }}</div></td>
                ...
                <div class="{{ $absentMonthly->hasPages() ? 'mt-4' : '' }}">
                    {{ $absentMonthly->links() }}
                </div>
            </div>
        </div>
    ...
    <x-modal name="import-absent-modal" closable="false" maxWidth="lg">
        <div class="flex justify-between items-start mb-4 space-x-4">
            <h4 class="font-semibold text-gray-600 pt-1 uppercase">
                Import Absent
            </h4>
            <button
                wire:click="$dispatch('close-modal', { name: 'import-absent-modal' })"
                type="button"
        ...
        <div class="flex-1 overflow-auto x-modal-content">
            <livewire:dashboard.absent.absent-import @absentImported="$refresh" />
        </div>
    </x-modal>

    <x-modal name="edit-absent-modal" closable="false" maxWidth="lg">
        <div class="flex justify-between items-start mb-4 space-x-4">
            <h4 class="font-semibold text-gray-600 pt-1 uppercase">
                Edit Absent 
            </h4>
            <button
                wire:click="$dispatch('close-modal', { name: 'edit-absent-modal' })"
                type="button"
                class="text-gray-600 hover:text-gray-900 bg-gray-50 hover:bg-gray-100 p-2 rounded-full inline-flex items-center"
            >
                <x-icon-close class="w-4 h-4"></x-icon-close>
