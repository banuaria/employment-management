<div>
    <form wire:submit="store" class="px-0.5">
        ...
        <div>
            <x-input-label for="title-store-create" :value="__('Title')" />
            <x-text-input wire:model="title" id="title-store-create" class="block mt-1 w-full" type="text" name="title" />
            <x-input-error :messages="$errors->get('title')" class="mt-2" />
        </div>

        <div class="mt-4">
            <x-input-label for="path-store-create" :value="__('Logo')" />
            <div class="my-1 px-11 py-4 relative flex justify-center border border-gray-200 rounded-md shadow-sm">
                @if (!$path)
                <x-icon-image wire:click="$dispatch('filemanager-picker', { to: 'dashboard.store.store-create', filetype: 'image', property: 'path' })" class="absolute top-1 left-2 w-7 h-7 text-gray-300 cursor-pointer"></x-icon-image>
                @endif
        ...
        <div class="mt-4">
            <x-input-label for="url-store-create" :value="__('Url')" />
            <x-text-input wire:model="url" id="url-store-create" class="block mt-1 w-full" type="text" name="url" />
