<div>
    <form wire:submit="store" class="px-0.5">
        <div>
            <x-input-label for="name-testimony-edit" :value="__('Name')" />
            <x-text-input wire:model="name" id="name-testimony-edit" class="block mt-1 w-full" type="text" name="name" />
            <x-input-error :messages="$errors->get('name')" class="mt-2" />
        </div>

        <div class="mt-4">
            <x-input-label for="profile-pic-testimony-edit" :value="__('Profile Pic')" />
            <div class="my-1 px-11 py-4 relative flex justify-center border border-gray-200 rounded-md shadow-sm">
                @if (!$profile_pic)
                <x-icon-image wire:click="$dispatch('filemanager-picker', { to: 'dashboard.testimony.testimony-edit', filetype: 'image', property: 'profile_pic' })" class="absolute top-1 left-2 w-7 h-7 text-gray-300 cursor-pointer"></x-icon-image>
                @endif
        ...
        <div class="mt-4">
            <x-input-label for="designation-testimony-edit" :value="__('Designation')" />
            <x-text-input wire:model="designation" id="designation-testimony-edit" class="block mt-1 w-full" type="text" name="designation" />
            <x-input-error :messages="$errors->get('designation')" class="mt-2" />
        </div>

        <div class="mt-4">
            <x-input-label for="content-testimony-edit" :value="__('Content')" />
            <x-textarea-input wire:model="content" id="content-testimony-edit" class="block mt-1 w-full" type="text" name="content" rows="3"></x-textarea-input>
