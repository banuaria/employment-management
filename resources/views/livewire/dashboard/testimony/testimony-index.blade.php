<div class="py-12">
    <div class="sm:px-6 lg:px-8">
        <div class="p-4 sm:p-8 bg-white shadow rounded-lg">
            <div class="w-full">
                        ...
                        <table class="w-full text-sm text-left rtl:text-right text-gray-500">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                                <tr>
                                    ...
                                    <th rowspan="1" class="px-6 py-3 border text-center whitespace-nowrap">Profile Pic</th>
                                    <th rowspan="1" class="px-6 py-3 border text-center whitespace-nowrap">Designation</th>
                                    <th rowspan="1" class="px-6 py-3 border text-center whitespace-nowrap">Content</th>
                            ...
                            <tbody>
                                @if (count($testimonies) > 0)
                                    ...
                                    <tr class="bg-white border-b">
                                        <td class="px-4 py-3 border text-center w-0"><div class="py-1.5">{{ $testimonies->firstItem() + $key }}</div></td>
                                        <td class="px-4 py-3 border-r">{{ $testimony->name }}</td>
                                        <td class="px-4 py-3 border-r text-center w-0">
                                            <div class="w-16 h-16">
                                            @if ($testimony->profile_pic)
                                                <img src="{{ asset($testimony->profile_pic) }}" class="w-full h-full object-cover" />
                                            @else
                                                <img src="{{ asset('images/default/profile_pic.png') }}" class="w-full h-full object-cover" />
                                            @endif
                                            </div>
                                        </td>
                                        <td class="px-4 py-3 border-r">{{ $testimony->designation }}</td>
                                        <td class="px-4 py-3 border-r">{{ $testimony->content }}</td>
                                        <td class="px-4 py-3 border-r w-0 whitespace-nowrap">
                                            <div class="flex flex-col justify-center items-center space-y-2">
                                                <div>{{ $testimony->createdBy->name }}</div>
                                                <div>{{ $testimony->created_at->format('d-m-Y H:i') }}</div>
                                            </div>
                                        </td>
                                        <td class="px-4 py-3 border-r w-0 whitespace-nowrap">
                                            <div class="flex flex-col justify-center items-center space-y-2">
                                                <div>{{ $testimony->updatedBy->name }}</div>
                                                <div>{{ $testimony->updated_at->format('d-m-Y H:i') }}</div>
                                            </div>
                                        </td>
                                        <td class="px-4 py-3 border-r w-0">
                                            <div class="flex justify-center items-center">
                                                <label class="relative cursor-pointer">
                                                    <input type="checkbox" value="" class="sr-only peer" wire:change="updateStatus({{ $testimony->id }}, $event.target.checked)" {{ $testimony->status ? 'checked' : ''}}>
                ...
                <div class="{{ $testimonies->hasPages() ? 'mt-4' : '' }}">
                    {{ $testimonies->links() }}
                </div>
            </div>
        </div>
    ...
    <x-modal name="create-testimony-modal" closable="false" maxWidth="lg">
        <div class="flex justify-between items-start mb-4 space-x-4">
            <h4 class="font-semibold text-gray-600 pt-1 uppercase">
                Add Testimony
            </h4>
            <button
                wire:click="$dispatch('close-modal', { name: 'create-testimony-modal' })"
                type="button"
        ...
        <div class="flex-1 overflow-auto x-modal-content">
            <livewire:dashboard.testimony.testimony-create @testimonyCreated="$refresh" />
        </div>
    </x-modal>

    <x-modal name="edit-testimony-modal" closable="false" maxWidth="lg">
        <div class="flex justify-between items-start mb-4 space-x-4">
            <h4 class="font-semibold text-gray-600 pt-1 uppercase">
                Edit Testimony
            </h4>
            <button
                wire:click="$dispatch('close-modal', { name: 'edit-testimony-modal' })"
                type="button"
                class="text-gray-600 hover:text-gray-900 bg-gray-50 hover:bg-gray-100 p-2 rounded-full inline-flex items-center"
            >
                <x-icon-close class="w-4 h-4"></x-icon-close>
        ...
        <div class="flex-1 overflow-auto x-modal-content">
            <livewire:dashboard.testimony.testimony-edit @testimonyEdited="$refresh" />
        </div>
    </x-modal>
</div>
