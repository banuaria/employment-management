        ...
        <div class="p-4 sm:p-8 bg-white shadow rounded-lg">
            <div class="w-full">
                <div class="flex justify-between items-center space-x-4 mb-4">
                    <a href="{{ route('cms.post.create') }}" wire:navigate>
                        <button type="button" class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest transition ease-in-out duration-1500">Create Post</button>
                    </a>
                            ...
                            <tbody>
                                @if (count($posts) > 0)
                                    ...
                                    <tr class="bg-white border-b">
                                        <td class="px-4 py-3 border text-center w-0"><div class="py-1.5">{{ $posts->firstItem() + $key }}</div></td>
                                        ...
                                        <td class="px-4 py-3 border-r">{{ $post->title }}</td>
                                        <td class="px-4 py-3 border-r">
                                            <div class="whitespace-nowrap">{{ $post->postCategory->title ?? '' }}</div>
                                        </td>
                                        <td class="px-4 py-3 border-r">
                                            <div class="whitespace-nowrap">{{ $post->postSubcategory->title ?? '' }}</div>
                                        </td>
                                        <td class="px-4 py-3 border-r">
                                        ...
                                        <td class="px-4 py-3 border-r w-0 whitespace-nowrap">
                                            <div class="flex flex-col justify-center items-center space-y-2">
                                                <div>{{ $post->updatedBy->name }}</div>
                                                <div>{{ $post->updated_at->format('d-m-Y H:i') }}</div>
                                            </div>
                                        </td>
                                        <td class="px-4 py-3 border-r w-0 whitespace-nowrap">
                                            <div class="flex flex-col justify-center items-center space-y-2">
                                                <div>{{ $post->publishedBy?->name }}</div>
                                                <div>{{ $post->published_at?->format('d-m-Y H:i') }}</div>
                                            </div>
                                        </td>
                                        <td class="px-4 py-3 border-r w-0">
                                            <div class="flex justify-center items-center">
                                                <label class="relative cursor-pointer">
                                                    <input type="checkbox" value="" class="sr-only peer" wire:change="updateHighlight({{ $post->id }}, $event.target.checked)" {{ $post->highlight ? 'checked' : ''}}>
                                        ...
                                        <td class="px-4 py-3 border-r w-0">
                                            <div class="flex justify-center items-center">
                                                <label class="relative cursor-pointer">
                                                    <input type="checkbox" value="" class="sr-only peer" wire:change="updateStatus({{ $post->id }}, $event.target.checked)" {{ $post->status ? 'checked' : ''}}>
                                                    <div class="relative w-11 h-6 bg-gray-200 rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-green-600"></div>
                                                </label>
                                            </div>
                                        </td>
                                        <td class="px-4 py-2 border-r w-0">
                                            <div class="flex justify-center items-center space-x-2">
                                                <a href="{{ route('cms.post.edit', ['id' => $post->id]) }}" wire:navigate>
                                                ...
                                                <button wire:click="$dispatch('alert-confirmation', { to: 'dashboard.post.post-index', data: { do: 'delete', 'id': {{ $post->id }} }, title: `Are you sure to delete post {{ $post->title }}?` })" type="button" class="inline-flex items-center px-4 py-2 bg-red-600 hover:bg-red-500 active:bg-red-700 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest transition ease-in-out duration-1500"><x-icon-trash class="w-3.5 h-3.5"></x-icon-trash></button>
                                            </div>
                                        </td>
                                    </tr>
                                    @endforeach
