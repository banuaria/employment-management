<div class="py-12">
                        ...
                        <div class="relative">
                            ...
                            <div class="flex-1">
                                <div class="w-full border border-primary-50 rounded-xl p-4">
                                    <div>
                                        <x-input-label for="title-post-create" :value="__('Title')" required/>
                                        <x-text-input wire:model="title" id="title-post-create" class="block mt-1 w-full" type="text" name="title" required/>
                                        <x-input-error :messages="$errors->get('title')" class="mt-2" />
                                    </div>
                                    <div class="mt-4">
                                        <x-input-label class="mb-2" for="content-post-create" :value="__('Content')" required/>
                                        <div wire:ignore>
                                            <x-textarea-input wire:model.change="content" id="content-post-create" class="block mt-1 w-full tinymce-textarea" tinymce-textarea-id="1" type="text" name="content" rows="3" required></x-textarea-input>
                                        </div>
                                        <x-input-error :messages="$errors->get('content')" class="mt-2" />
                                    </div>
                                    <div class="mt-4">
                                        <x-input-label for="meta-title-post-create" :value="__('Meta Title')" />
                                        <x-text-input wire:model="meta_title" id="meta-title-post-create" class="block mt-1 w-full" type="text" name="meta_title" />
                                        <x-input-error :messages="$errors->get('meta_title')" class="mt-2" />
                                    </div>
                                    <div class="mt-4">
                                        <x-input-label for="meta-desc-post-create" :value="__('Meta Description')" />
                                        <x-textarea-input wire:model="meta_desc" id="meta-desc-post-create" class="block mt-1 w-full" type="text" name="meta_desc" rows="3"></x-textarea-input>
                                        <x-input-error :messages="$errors->get('meta_desc')" class="mt-2" />
                                    </div>
                                    <div class="mt-4">
                                        <x-input-label for="meta-keywords-post-create" :value="__('Meta Keywords')" />
                                        <x-text-input wire:model="meta_keywords" id="meta-keywords-post-create" class="block mt-1 w-full" type="text" name="meta_keywords" />
                ...
                <div class="w-full xl:w-2/6 flex flex-col-reverse xl:flex-col gap-y-4">
                    <div class="p-4 sm:p-8 bg-white shadow rounded-lg w-full">
                        <div class="relative">
                            ...
                            <div class="flex justify-between space-x-4">
                                ...
                                <x-secondary-button wire:click="$dispatch('open-modal', { name: 'post-create-preview-modal' })" class="flex-1 flex justify-center">Preview</x-secondary-button>
                            </div>
                        </div>
                    </div>
                    <div class="p-4 sm:p-8 bg-white shadow rounded-lg w-full">
                        <div class="relative">
                            ...
                            <div class="px-0.5">
                                <div>
                                    ...
                                    <x-input-label for="thumbnail-post-create" :value="__('Thumbnail')" />
                                    <div class="my-1 px-11 py-4 relative flex justify-center border border-gray-200 rounded-md shadow-sm">
                                        @if (!$thumbnail)
                                        <x-icon-image wire:click="$dispatch('filemanager-picker', { to: 'dashboard.post.post-create', filetype: 'image', property: 'thumbnail' })" class="absolute top-1 left-2 w-7 h-7 text-gray-300 cursor-pointer"></x-icon-image>
                                        @endif
                                ...
                                <div class="mt-4">
                                    <x-input-label for="cover-post-create" :value="__('Cover')" />
                                    <div class="my-1 px-11 py-4 relative flex justify-center border border-gray-200 rounded-md shadow-sm">
                                        @if (!$cover)
                                        <x-icon-image wire:click="$dispatch('filemanager-picker', { to: 'dashboard.post.post-create', filetype: 'image', property: 'cover' })" class="absolute top-1 left-2 w-7 h-7 text-gray-300 cursor-pointer"></x-icon-image>
                                        @endif
                    ...
                    <div class="p-4 sm:p-8 bg-white shadow rounded-lg w-full">
                        <div class="relative">
                            ...
                            <div class="px-0.5">
                                <div>
                                    <x-input-label for="post-category-id-post-create" :value="__('Category')" />
                                    <select wire:model.change="post_category_id" id="post-category-id-post-create" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                ...
                                <div class="mt-4">
                                    <x-input-label for="post-subcategory-id-post-create" :value="__('Subcategory')" />
                                    <select wire:model="post_subcategory_id" id="post-subcategory-id-post-create" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
    ...
    <x-modal name="post-create-preview-modal" closable="true" maxWidth="3xl">
        <div class="h-screen flex flex-col overflow-hidden">
            <div class="flex justify-between items-start mb-4 space-x-4">
                ...
                <button
                    wire:click="$dispatch('close-modal', { name: 'post-create-preview-modal' })"
                    type="button"
            ...
            <div class="flex-1 overflow-auto x-modal-content border border-primary-50 rounded-xl p-4">
                <div>
                    ...
                    <div class="flex items-center space-x-1.5 mb-4">
                        <div class="text-sm font-bold text-gray-600">{ Here is Writer }</div>
                        <div class="text-sm font-thin text-gray-600">{ Here is Published At }</div>
                    </div>
                    @if ($cover)
