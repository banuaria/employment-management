<div>
    <form wire:submit="store" class="px-0.5">
        ...
        <div>
            <x-input-label for="name-product-category-edit" :value="__('Title')" />
            <x-text-input wire:model="title" id="name-product-category-edit" class="block mt-1 w-full" type="text" name="title" />
            <x-input-error :messages="$errors->get('title')" class="mt-2" />
        </div>

        <div class="mt-4">
            <x-input-label for="cover-product-category-edit" :value="__('Cover')" />
            <div class="my-1 px-11 py-4 relative flex justify-center border border-gray-200 rounded-md shadow-sm">
                @if (!$cover)
                <x-icon-image wire:click="$dispatch('filemanager-picker', { to: 'dashboard.product-category.product-category-edit', filetype: 'image', property: 'cover' })" class="absolute top-1 left-2 w-7 h-7 text-gray-300 cursor-pointer"></x-icon-image>
                @endif
