<div>
    <form wire:submit="store" class="px-0.5">
        ...
        <div>
            <x-input-label for="name-feature-edit" :value="__('Title')" />
            <x-text-input wire:model="title" id="name-feature-edit" class="block mt-1 w-full" type="text" name="title" />
            <x-input-error :messages="$errors->get('title')" class="mt-2" />
        </div>

        <div class="mt-4">
            <x-input-label for="path-feature-edit" :value="__('Cover')" />
            <div class="my-1 px-11 py-4 relative flex justify-center border border-gray-200 rounded-md shadow-sm">
                @if (!$path)
                <x-icon-image wire:click="$dispatch('filemanager-picker', { to: 'dashboard.feature.feature-edit', filetype: 'image', property: 'path' })" class="absolute top-1 left-2 w-7 h-7 text-gray-300 cursor-pointer"></x-icon-image>
                @endif
        ...
        <div class="mt-4">
            <x-input-label class="mb-2" for="desc-feature-edit" :value="__('Description')" />
            <div wire:ignore>
                <x-textarea-input wire:model.change="desc" id="desc-feature-edit" class="block mt-1 w-full tinymce-textarea" tinymce-textarea-id="1" type="text" name="desc" rows="3"></x-textarea-input>
            </div>
